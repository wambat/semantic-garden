(ns semantic-garden.styles.form
...
(defstyle elements
  [:.ui.form {:position :relative
   ...
   [:.field {:clear :both
             :margin fieldMargin}]]
  [(sel/> :.ui.form :p) {:margin paragraphMargin}])


;;Then you can
(comment
  (css-branch elements {:.ui.form {:.field "*"}})
  ;;or
  (css-element elements [:.ui.form [:.field]])
  )

;;to import only needed defs
/*--------------------
        Field
---------------------*/
